...
)
{
    ...
    {
    	Print(L"GetIdeRegisterIoAddr IdeRegisters == NULL\n");
    ...
    if (EFI_ERROR (Status))
    {
    	Print(L"GetIdeRegisterIoAddr Pci.Read failed with error 0x%x\n", Status);
        return Status;
    }

    BusMasterBaseAddr    = (UINT16) ((PciData.Device.Bar[4] & 0x0000fff0));
    Print(L"GetIdeRegisterIoAddr BusMasterBaseAddr 0x%x, ClassCode %x,%x,%x, vid %x:%x\n", 
    	BusMasterBaseAddr, 
    	PciData.Hdr.ClassCode[0], PciData.Hdr.ClassCode[1], PciData.Hdr.ClassCode[2],
    	PciData.Hdr.VendorId, PciData.Hdr.DeviceId);
    Print(L"GetIdeRegisterIoAddr PciData 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", 
    	PciData.Device.Bar[0], PciData.Device.Bar[1], 
    	PciData.Device.Bar[2], PciData.Device.Bar[3],
...
)
{
    ...
    if (EFI_ERROR (Status))
    {
    	Print(L"AtaIssueCommand failed with error 0x%x\n", Status);
    ...
    while (WordCount < RShiftU64(ByteCount, 1))
    {
        ...
        if (EFI_ERROR (Status))
        {
        	Print(L"DRQReady2 failed with error 0x%x\n", Status);
            Status = EFI_DEVICE_ERROR;
            goto Exit;
        }
        ...
        if (EFI_ERROR (Status))
        {
        	Print(L"CheckStatusRegister failed with error 0x%x\n", Status);
            Status = EFI_DEVICE_ERROR;
            goto Exit;
        }
    ...
    if (EFI_ERROR (Status))
    {
    	Print(L"DRQClear failed with error 0x%x\n", Status);
        //Status = EFI_DEVICE_ERROR;
        Status = EFI_SUCCESS;
        goto Exit;
    }
...
)
{
    ...
    for (IdeDevice = 0; IdeDevice < EfiIdeMaxDevice; IdeDevice++)
    {
        ...
        Print(L"IdeWritePortB: %x, %x\n", IdeRegisters->CmdOrStatus, ATA_CMD_EXEC_DRIVE_DIAG);

        Status = WaitForBSYClear (PciIo, IdeRegisters, 350000000);
        if (EFI_ERROR (Status))
        {
            Print(L"New detecting method: WaitForBSYClear: Status: %d\n", Status);
            continue;
        }
        ...
        {
        	Print(L"Found Harddisk.\n");
            DeviceType = EfiIdeHarddisk;
        }
        else
        {
        	Print(L"SectorCountReg %d\n", SectorCountReg);
        	Status = EFI_DEVICE_ERROR;
            continue;
        }
        ...
        {
            //void * ptrSerial = (void *)Buffer.AtaData.SerialNo;
            //void * ptrFw = (void *)Buffer.AtaData.FirmwareVer;
            //void * ptrModel = (void *)Buffer.AtaData.ModelName;
            //EFI_IDENTIFY_DATA ideinfo;
            //char ideinfo[sizeof(EFI_IDENTIFY_DATA)];
            //memcpy(&ideinfo, &Buffer, sizeof(Buffer));
            //Swap((char*)&ideinfo, sizeof(EFI_IDENTIFY_DATA));
	        //Print((CHAR16 *)L"Firmware: %s\n", ptrFw);
    	    //Print((CHAR16 *)L"Serial:   %s\n", ptrSerial);
        	//Print((CHAR16 *)L"Model:    %s\n", ptrModel);
        	//PrintBinary(&ideinfo, 0x80);
        	//if(stristr(ideinfo.AtaData.ModelName, "snapshot") == NULL)
            WORD *ideInfo = (WORD*)&Buffer;
        	if(!(ideInfo[129] & 1))
        	{
        		Status = EFI_DEVICE_ERROR;
        		continue;
        ...
        if (EFI_ERROR (Status))
        {
        	Print(L"CreateNewDeviceInfo failed with error 0x%x\n", Status);
        }
        else
        	break;
    }
...
)
{
    ...
    for (IdeChannel = 0; IdeChannel < Channel; IdeChannel++)
    {
        ...
        //
        Status = DetectAndConfigIdeDevice (Instance, IdeChannel);
        if (!EFI_ERROR (Status))
        	break;
    }
