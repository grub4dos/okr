...
)
{
    ...
    case EfiAtaIdeMode:
        ...
        if (EFI_ERROR (Status))
        {
        	Print(L"GetIdeRegisterIoAddr failed with error 0x%x\n", Status);
            return Status;
        }
...
InitializeAtaAtapiPassThru (
    ...
    ATA_ATAPI_PASS_THRU_INSTANCE **ppInstance
)
{
    //EFI_STATUS              Status;
    ...
    Status = gBS->LocateHandleBuffer(ByProtocol, &gEfiIdeControllerInitProtocolGuid,
            NULL, &HandleCount, &Handles);
    if (!EFI_ERROR (Status))
    {
    	Print(L"EfiIdeControllerInitProtocol HandleCount is: %d, status %d\n", HandleCount, Status);
        for (i=0;i<HandleCount;i++)
        {
            Print(L"EfiIdeControllerInitProtocol %d, handle %x\n", i, Handles[i]);
            //Status = gBS->HandleProtocol(Handles[i], &gEfiBlockIoProtocolGuid, &blockIo);
            //Print(L"EfiBlockIoProtocol returns %d, handle %x\n", Status, Handles[i]);
            //Status = gBS->HandleProtocol(Handles[i], &gEfiPciIoProtocolGuid, &PciIo);
            //Print(L"gEfiPciIoProtocolGuid returns %d, handle %x\n", Status, Handles[i]);
            Status = AtaAtapiPassThruStart(Handles[i], ppInstance);
            if (!EFI_ERROR (Status))
                break;
        }
    	if (Handles)
    ...
    //if (!EFI_ERROR (Status))
    //{
    //    Print(L"EfiPciIoProtocol HandleCount is: %d, status %d\n", HandleCount, Status);
    //
    //    for (i=0;i<HandleCount;i++)
    //    {
    //        Print(L"AtaAtapiPassThruStart %d, handle %x\n", i, Handles[i]);
    //        Status = AtaAtapiPassThruStart(Handles[i], ppInstance);
    //        if (!EFI_ERROR (Status))
    //            break;
    //    }
...
)
{
    ...
    Print(L"AtaAtapiPassThruSupported, this %p, controller %d\n", This, Controller);
    //
    ...
    if (EFI_ERROR (Status))
    {
        ...
        //
        Print(L"AtaAtapiPassThruSupported(1) status %d\n", Status);
        return Status;
    }
    //
    ...
    if (EFI_ERROR (Status))
    {
        ...
        // EFI_ALREADY_STARTED is also an error
        //
        Print(L"AtaAtapiPassThruSupported(2) status %d\n", Status);
        return Status;
    }
    ...
    if (EFI_ERROR (Status))
    {
        Print(L"AtaAtapiPassThruSupported(3) status %d\n", Status);
        return Status;
    }
    //
    ...
    if (EFI_ERROR (Status))
    {
        Print(L"AtaAtapiPassThruSupported(PciRead) status %d\n", Status);
        return EFI_UNSUPPORTED;
    }

    if (IS_PCI_IDE (&PciData) || IS_PCI_SATADPA (&PciData))
    {
        Print(L"AtaAtapiPassThruSupported isIDE %d, isATA %d\n",
                IS_PCI_IDE (&PciData), IS_PCI_SATADPA (&PciData));
        return EFI_SUCCESS;
    }

    Print(L"AtaAtapiPassThru Not Supported, isIDE %d, isATA %d\n",
            IS_PCI_IDE (&PciData), IS_PCI_SATADPA (&PciData));
...
)
{
    ...
    if (EFI_ERROR (Status))
    {
        Print (L"Get Pci_Io Protocol Error, Status=%d\n", Status);
        goto ErrorExit;
    }
    ...
    if (EFI_ERROR (Status))
    {
        Print (L"PciIo->Attributes(Get) Error, Status=%d\n", Status);
        goto ErrorExit;
    }
    ...
    if (EFI_ERROR (Status))
    {
        Print (L"PciIo->Attributes(Enable) Error, Status=%d\n", Status);
        goto ErrorExit;
    }
    ...
    if (!EFI_ERROR (Status))
    {
            ...
            || ClassCode == PCI_CLASS_MASS_STORAGE_SATADPA)
        {
            ...
            if ( ClassCode == PCI_CLASS_MASS_STORAGE_IDE)
            {
                ...
                //
                Print (L"EnumerateAttachedDevice PCI_CLASS_MASS_STORAGE_IDE\n");
            ...
            else
            {
                ...
                //
                Print (L"EnumerateAttachedDevice PCI_CLASS_MASS_STORAGE_SATADPA\n");
    ...
    if (EFI_ERROR (Status))
    {
        Print (L"EnumerateAttachedDevice Error, Status=%d\n", Status);
        goto ErrorExit;
    }
...
)
{
    ...
    Print(L"==AtaAtapiPassThru Stop== Controller = %x\n", Controller);
...
)
{
    ...
    Print(L"CreateNewDeviceInfo Port %d, PortMultiplier %d\n", Port, PortMultiplier);
...
)
{
    ...
    {
    	Print(L"AtaPassThruPassThru, InDataBuffer align error \n");
        return EFI_INVALID_PARAMETER;
    }

    if ((This->Mode->IoAlign > 1) && !IS_ALIGNED(Packet->OutDataBuffer, This->Mode->IoAlign))
    {
    	Print(L"AtaPassThruPassThru, OutDataBuffer align error \n");
        return EFI_INVALID_PARAMETER;
    }

    if ((This->Mode->IoAlign > 1) && !IS_ALIGNED(Packet->Asb, This->Mode->IoAlign))
    {
    	Print(L"AtaPassThruPassThru, Asb align error \n");
        return EFI_INVALID_PARAMETER;
    }
    ...
    if (Node == NULL)
    {
    	Print(L"AtaPassThruPassThru, node == null \n");
        return EFI_INVALID_PARAMETER;
    }
    ...
    {
    	Print(L"AtaPassThruPassThru, in %d, out %d, maxsector %d \n", 
    		Packet->InTransferLength, Packet->OutTransferLength, MaxSectorCount);
