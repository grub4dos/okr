...
BOOL GetNtfsFileRecord(DWORD Inode, void * RecordBuffer)
{
	...
	if(g_Partitions[g_PartitionIndex].SectorsPerClr <= g_Partitions[g_PartitionIndex].SectorsPerFileRecord)
	{
		...
		offset = 0;
		MyLog(LOG_DEBUG, L" zwang GetNtfsFileRecord1111 fail\n");
	}
	else
	{
		...
		count = 1;
		MyLog(LOG_DEBUG, L" zwang GetNtfsFileRecord2222 fail\n");
	}
...
DWORD FindInodeFromNameInRoot(char *FileName, FILE_RECORD_HEADER *InodeBuffer)
{
    ...
    //MyLog(LOG_DEBUG, L" zwang GetNtfsFileRecord fail\n");   //删除此行代码，解决备份数据大小异常问题
		return 0;
...
BOOL LoadMFT(WORD PartitionIndex)
{
	...
	if(!DiskRead(g_Partitions[g_PartitionIndex].DiskNumber, sector, 
...
DWORD DumpFileSectorListNtfs(WORD PartitionIndex, char *FileName, SIZE_L *pList, DWORD MaxListNumber)
{
    ...
    //MyLog(LOG_DEBUG, L" zwang LoadMFT fail\n"); //删除此行代码，解决备份数据大小异常问题
		goto failed;
	...
	MyLog(LOG_DEBUG, L" zwang FindInodeFromNameInRoot fail\n");
	if(inode == 0)
		goto failed;
